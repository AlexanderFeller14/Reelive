...
test('the plus button in the header leads to creating a trip', async () => {
  (fetchTrips as jest.Mock).mockResolvedValue(loaded([]));
  await wrap();
...
// Since the mockup (2026-08-27) the running section is always titled: the
// section name doubles as the status the wax seal used to claim.
test('a running trip stands under Aktive Reise even without planned ones', async () => {
  (fetchTrips as jest.Mock).mockResolvedValue(loaded([trip]));
  await wrap();
  await screen.findByText('Norwegen mit dem Camper');
  expect(screen.getByText('Aktive Reise')).toBeTruthy();
  expect(screen.queryByText('Geplant')).toBeNull();
});

test('several running trips pluralize the section title', async () => {
  (fetchTrips as jest.Mock).mockResolvedValue(loaded([trip, { ...trip, id: 't9', name: 'Ligurien' }]));
  await wrap();
  await screen.findByText('Ligurien');
  expect(screen.getByText('Aktive Reisen')).toBeTruthy();
});

test('the running card carries status badge, remaining days and the people row', async () => {
  (fetchTrips as jest.Mock).mockResolvedValue(loaded([trip]));
  await wrap();
  await screen.findByText('Norwegen mit dem Camper');
  expect(screen.getByText('Aktiv')).toBeTruthy();
  // Trip ends 2026-08-14, the pinned clock says 2026-08-10.
  expect(screen.getByText('Noch 4 Tage')).toBeTruthy();
  expect(screen.getByText('Lea + 1 weitere')).toBeTruthy();
  expect(screen.getByText('7 Momente')).toBeTruthy();
});

test('a planned trip splits the list into Aktive Reise above and Geplant below', async () => {
  (fetchTrips as jest.Mock).mockResolvedValue(loaded([planned, trip]));
  await wrap();
  expect(await screen.findByText('Norwegen mit dem Camper')).toBeTruthy();
  expect(screen.getByText('Island im Winter')).toBeTruthy();
  const titles = screen.getAllByText(/^(Aktive Reise|Geplant)$/).map((n) => n.props.children);
  expect(titles).toEqual(['Aktive Reise', 'Geplant']);
});
...
test('with only planned trips the Geplant section stands alone, no empty state', async () => {
  ...
  expect(screen.queryByText('Aktive Reise')).toBeNull();
